...
struct ProbeSession <: AbstractSession end
struct Probe1v1Session <: AbstractSession end
struct ProbeCompound1v1Session <: AbstractSession end
struct Training1v1Session <: AbstractSession end
struct Training4v1Session <: AbstractSession end
struct TrainingCompound1v1Session <: AbstractSession end

"""
Sorting KLimbic files in chronological order.
The day-month-year date is assumed to always be on the file name.
"""
...
function read_JBT(dir; reversed=false)
	...
	stype_d = Dict(
					"Probe tone midpoint only testing" => (ProbeSession(), 20),
					"1 vs 1 Probe Test" => (Probe1v1Session(), 22),
					"Discrimination training" => (Training1v1Session(), 18),
					"Pure tones testing" => (Training4v1Session(), 18),
					"1vs1" => (ProbeCompound1v1Session(), 32),
					"Discrimination" => (TrainingCompound1v1Session(), 42)
					)
...
function append_subj!(df, D, subj_id, date, ::AbstractSession; col_offset=0, reversed=false)
    @warn "Session type not found for subject $subj_id at $date. \n
        Please look at the AC Comment row of the KLimbic output file"
    allowmissing!(df)
    append!(
        df,
        DataFrame(
...
function append_subj!(
					...
					subj_id::String,
					date,
					t::Union{Training1v1Session, Training4v1Session};
					col_offset=0,
					reversed=false
					)
	...
	if mod(id_number, 2) == 0
		...
		outcome[mask_corr_cue1] .= 1
		if typeof(t) == Training1v1Session
			outcome[mask_corr_cue2] .= 1
		else
	...
	st = typeof(t)==Training1v1Session ? "train_1vs1" : "train_4vs1"
